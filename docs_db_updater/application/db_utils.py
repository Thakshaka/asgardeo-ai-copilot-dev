...
def delete_records(filename, db_client):
    ...
    if db_type == "pgvector":
        ...
        primary_keys = []
        with db_client.conn.cursor() as cursor:
            cursor.execute(
                f"SELECT id FROM {os.environ.get(const.DOCS_COLLECTION)} WHERE metadata->>'filename' = %s",
                (filename,)
            )
            ...
            if primary_keys:
                ...
                cursor.execute(
                    f"DELETE FROM {os.environ.get(const.DOCS_COLLECTION)} WHERE id IN ({placeholders})",
                    primary_keys
                )
...
def add_records(filename, file_content, db_client, embed):
    ...
    if db_type == "pgvector":
        ...
        for chunk in chunked_docs:
            metadata = chunk[const.ASGARDEO_METADATA]
