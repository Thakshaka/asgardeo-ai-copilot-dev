...
def create_commits_collection(milvus_client):
    ...
    schema.add_field(field_name=const.LAST_UPDATED_COMMIT, datatype=DataType.VARCHAR, max_length=100)
    ...
    while attempt < retries:
        try:
            cached_commit = milvus_client.query(
                ...
                output_fields=[const.LAST_UPDATED_COMMIT, const.LAST_UPDATER_VERSION]
            )
            if cached_commit:
...
def update_last_updated_commit(commit_sha, milvus_client):
    ...
    payload = {
        ...
        const.LAST_UPDATED_COMMIT: commit_sha,
        const.LAST_UPDATER_VERSION: const.UPDATER_VERSION
    }
    response = milvus_client.upsert(collection_name=os.environ.get(const.COMMITS_COLLECTION), data=payload)
...
def check_collection_existence(milvus_client):
    ...
    """
    has = milvus_client.has_collection(collection_name=os.environ.get(const.COMMITS_COLLECTION))
    if not has:
        return False
    return True
